...
def gptq_gemm_kernel(
    ...
    Note: Input activations and weights should be pre-processed
    (reordered/shuffled) if g_idx is present. This kernel assumes inputs
    ...
    b_ptrs = (b_qweight_ptr + offsets_k[:, None] // 8 * N +
              offsets_bn[None, :])
    ...
    for _ in range(0, tl.cdiv(K, BLOCK_SIZE_K * SPLIT_K)):
        ...
        shifts = tl.broadcast_to(k_shifts[:, None],
                                 (BLOCK_SIZE_K, BLOCK_SIZE_N))
        b_qweight = (b_qweight >> shifts) & 0xF

        # Convert from uint4 [0, 15] to int4 [-8, 7] for GPTQ symmetric
    ...
    c_ptrs = (c_ptr + pid_z * N * M + N * offs_cm[:, None] +
              offs_cn[None, :])
...
def gptq_gemm_triton(
    ...
    # (should be shuffled if g_idx present)
    ...
    Args:
        input: Input activations [M, K], float16. Should be reordered using
               g_idx if activation reordering is enabled.
        qweight: Quantized weights [K // 8, N], int32. Should be shuffled
                 using gptq_shuffle if activation reordering is enabled.
